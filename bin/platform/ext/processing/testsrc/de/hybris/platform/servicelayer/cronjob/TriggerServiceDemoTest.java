...
public class TriggerServiceDemoTest extends ServicelayerBaseTest
{
	 ...
	 */
	private TriggerModel configureNewTrigger(final JobModel job, final int second, final int minute, final int hour, final int day,
			final int month, final int year, final List daysOfWeek, final boolean relative)
	{
		final TriggerModel trigger = modelService.create(TriggerModel.class);
		...
		trigger.setActivationTime(triggerService
				.getNextTime(trigger, Calendar.getInstance(i18NService.getCurrentTimeZone(), i18NService.getCurrentLocale()))
				.getTime());
